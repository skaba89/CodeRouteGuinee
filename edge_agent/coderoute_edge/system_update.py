...
def _restart_systemd(service_name: str) -> None:
    subprocess.run(["systemctl", "restart", service_name], check=True, timeout=45, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def _verify_staged_with_local_trust(staged: dict[str, Any], staging_root: Path, node_id: str, center_id: str, current_version: str) -> dict[str, Any]:
    trust_store = Path(os.environ.get("CODEROUTE_EDGE_RELEASE_TRUST_PATH", "/etc/coderoute-edge/release-trust.json"))
    return verify_staged_release_for_root(staged, release_root=staging_root, trust_store_path=trust_store, expected_node_id=node_id, expected_center_id=center_id, expected_current_version=current_version)


def _failed_receipt(staging_root: Path, staged: dict[str, Any], *, error: str, rollback: dict[str, Any] | None) -> dict[str, Any]:
    receipt = {
...
def _prepare_offline_runtime(target_root: Path, runtime_python: str) -> dict[str, Any]:
    ...
    if venv.exists():
        shutil.rmtree(venv)
    subprocess.run([runtime_python, "-m", "venv", str(venv)], check=True, timeout=120, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    pip = venv / "bin" / "pip"
    subprocess.run([str(pip), "install", "--disable-pip-version-check", "--no-cache-dir", "--no-index", "--find-links", str(wheelhouse), "--require-hashes", "-r", str(lock)], check=True, timeout=300, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ready = {"requirements_lock_sha256": lock_sha, "python": runtime_python, "venv_python": str((venv / "bin" / "python").resolve()), "wheel_count": len(list(wheelhouse.glob("*.whl")))}
    _json_atomic(marker, ready)
    return ready


def apply_system_update_transaction(config: EdgeAgentConfig, *, emergency_window_bypass: bool = False, restart_service: RestartService = _restart_systemd, health_probe: HealthProbe = _probe_health, runtime_prepare: RuntimePrepare = _prepare_offline_runtime, staged_verifier: StagedVerifier = _verify_staged_with_local_trust) -> dict[str, Any]:
    state = assert_safe_maintenance(config.database_path, config.maintenance_windows, config.maintenance_timezone, bypass_window=emergency_window_bypass)
    staging_root = getattr(config, "release_staging_dir", None) or config.release_dir
    staged = _read_staged(staging_root)
    root_verification = staged_verifier(staged, staging_root, config.node_id, config.center_id, config.software_version)
    ...
    try:
        ...
        return {"ok": False, "phase": "runtime" if rollback_receipt else "critical", "receipt": failed, "rollback": rollback_receipt, "root_verification": root_verification, "current_switched": switched, "quiescent": state.quiescent}
    ...
    try:
        ...
        if running_version != expected_version:
            raise RuntimeError(f"Version démarrée {running_version or 'inconnue'} différente de la version attendue {expected_version}")
        _handoff_receipt(staging_root, config.release_dir, installed)
        return {"ok": True, "phase": "confirmed", "receipt": installed, "root_verification": root_verification, "runtime": runtime, "health": health, "quiescent": state.quiescent}
        ...
        try:
            ...
            if rollback_version != expected_rollback:
                raise RuntimeError(f"Rollback démarré en {rollback_version or 'inconnue'} au lieu de {expected_rollback}")
        except Exception as exc:
            rollback_error = exc
        ...
        failed = _failed_receipt(staging_root, staged, error=error, rollback=rollback_receipt)
        _handoff_receipt(staging_root, config.release_dir, failed)
        return {"ok": False, "phase": "rolled_back" if rollback_receipt else "critical", "receipt": failed, "rollback": rollback_receipt, "root_verification": root_verification, "quiescent": state.quiescent}
