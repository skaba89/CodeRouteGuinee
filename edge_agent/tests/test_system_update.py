...
def test_system_update_confirms_exact_running_version(tmp_path: Path) -> None:
    ...
    restarts: list[str] = []
    result = apply_system_update_transaction(config, emergency_window_bypass=True, restart_service=lambda service: restarts.append(service), health_probe=lambda *_args: {"status": "ok", "software_version": "edge-agent-0.4.0"}, runtime_prepare=_runtime, staged_verifier=_trusted_for_transaction)
...
def test_copy_failure_before_switch_keeps_current_and_previous_unchanged(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(system_update_module, "_copy_into_root_workspace", lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("simulated disk failure before current switch")))
    result = apply_system_update_transaction(config, emergency_window_bypass=True, restart_service=lambda _service: None, health_probe=lambda *_args: {"status": "ok", "software_version": "edge-agent-0.4.0"}, runtime_prepare=_runtime, staged_verifier=_trusted_for_transaction)
...
def test_runtime_failure_after_switch_rolls_back_without_restart(tmp_path: Path) -> None:
    ...
    restarts: list[str] = []
    result = apply_system_update_transaction(config, emergency_window_bypass=True, restart_service=lambda service: restarts.append(service), health_probe=lambda *_args: {"status": "ok", "software_version": "edge-agent-0.4.0"}, runtime_prepare=lambda *_args: (_ for _ in ()).throw(RuntimeError("simulated offline runtime failure")), staged_verifier=_trusted_for_transaction)
    assert result["ok"] is False
    ...
    assert previous_target is not None and previous_target.name == "edge-agent-0.4.0"
    receipt = json.loads((config.release_staging_dir / "install-receipt.json").read_text())
...
def test_health_version_mismatch_rolls_back_and_marks_bad_release_failed(tmp_path: Path) -> None:
    ...
    bad = _stage(config.release_staging_dir, "edge-agent-0.4.0")
    restarts: list[str] = []
    probes = iter([{"status": "ok", "software_version": "edge-agent-0.3.0"}, {"status": "ok", "software_version": "edge-agent-0.3.0"}])
    result = apply_system_update_transaction(config, emergency_window_bypass=True, restart_service=lambda service: restarts.append(service), health_probe=lambda *_args: next(probes), runtime_prepare=_runtime, staged_verifier=_trusted_for_transaction)
    assert result["ok"] is False
...
def test_first_install_runtime_failure_without_previous_is_critical(tmp_path: Path) -> None:
    ...
    config = _config(tmp_path, database)
    bad = _stage(config.release_staging_dir, "edge-agent-0.4.0")
    result = apply_system_update_transaction(config, emergency_window_bypass=True, restart_service=lambda _service: None, health_probe=lambda *_args: {"status": "ok", "software_version": "edge-agent-0.4.0"}, runtime_prepare=lambda *_args: (_ for _ in ()).throw(RuntimeError("simulated first-install runtime failure")), staged_verifier=_trusted_for_transaction)
    assert result["ok"] is False
    assert result["phase"] == "critical"
    assert result["current_switched"] is True
    assert result["rollback"] is None
...
def test_active_exam_blocks_transaction_before_copy_link_switch_or_restart(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="examen"):
        apply_system_update_transaction(config, emergency_window_bypass=True, restart_service=lambda _service: None, health_probe=lambda *_args: {"status": "ok", "software_version": "edge-agent-0.4.0"}, runtime_prepare=_runtime, staged_verifier=_trusted_for_transaction)
