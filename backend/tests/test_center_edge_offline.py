...
def _seed_attempt() -> tuple[Center, ExamAttempt, list[Question]]:
    ...
    with SessionLocal() as db:
        db.add_all([center, session, candidate, q1, q2, attempt, trace])
        db.commit()
        for obj in (center, attempt, q1, q2):
            db.refresh(obj)
            db.expunge(obj)
    return center, attempt, [q1, q2]
...
def test_edge_offline_lease_and_delayed_sync_scores_centrally() -> None:
    ...
    with TestClient(app) as client:
        ...
        center, attempt, questions = _seed_attempt()
        ...
        # Simulation réelle : le WAN a été coupé, le serveur central a entre-temps
        # marqué la tentative expirée. Le journal Edge prouve néanmoins que la
        # fermeture locale a eu lieu dans la durée autorisée.
        with SessionLocal() as db:
        ...
        # Retry réseau : nouvelle enveloppe signée/séquence, même journal.
