...
class Settings(BaseSettings):
    ...
    # P10 — état partagé multi-instance. Redis/Valkey ne stocke aucune donnée d'examen.
    redis_url: str = ""
    ...
    def validate_production_secrets(self) -> None:
        ...
        def placeholder(value: str, name: str) -> None:
            if not value or not value.strip() or any(p in value for p in placeholder_substrings):
                errors.append(f"{name} est vide ou placeholder — définir dans le gestionnaire d'environnement")

        def strength(value: str, name: str) -> None:
            if not value or not value.strip():
                return
            if len(value) < min_secret_length:
                errors.append(f"{name} est trop court ({len(value)} caractères, minimum {min_secret_length})")
            if value.strip().lower() in weak_values:
                errors.append(f"{name} est une valeur faible et devinable")
            if len(set(value)) < 8:
                errors.append(f"{name} a trop peu de caractères distincts")

        placeholder(self.secret_key, "SECRET_KEY")
        placeholder(self.csrf_secret, "CSRF_SECRET")
        placeholder(self.database_url, "DATABASE_URL")
        strength(self.secret_key, "SECRET_KEY")
        strength(self.csrf_secret, "CSRF_SECRET")
        ...
        if errors:
            ...
            raise RuntimeError(f"Configuration de production invalide :\n{msg}\nCorriger les variables d'environnement puis redéployer.")
