...
export type MediaAsset = {
  ...
  regulatory_authority_reference?: string | null;
...
export function getMediaUploadTarget(
  mediaType: MediaType,
  filename: string,
  contentType: string,
  provider?: string,
  ...
  return postPrivateJson<UploadTarget>(`/api/v1/media-library/upload-target${suffix}`, {
    media_type: mediaType,
    filename,
...
export function approveMediaRegulatory(mediaId: string, authorityReference: string, reason: string): Promise<MediaAsset> {
  return postPrivateJson<MediaAsset>(`/api/v1/media-library/assets/${encodeURIComponent(mediaId)}/regulatory/approve`, {
...
export async function inspectMediaFile(file: File, mediaType: MediaType): Promise<{
  width?: number;
  height?: number;
  ...
  if (mediaType === 'video' || mediaType === 'audio') {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        media.onerror = () => reject(new Error('Média illisible'));
        media.src = url;
      });
      return {
...
export async function uploadMediaFile(file: File, mediaType: MediaType, provider?: string): Promise<{
  ...
  if (target.method === 'POST') {
    ...
    return {
      ...
      storageKey: payload.public_id ?? target.storage_key,
      secureUrl,
    };
  }
  ...
  const cleanUrl = target.upload_url.split('?')[0];
  return {
    provider: target.provider,
